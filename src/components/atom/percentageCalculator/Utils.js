    ...
    weekWiseData &&
      Object.keys(weekWiseData).map((weekKeyName, index) => {
        ...
        for (let iterations = index; iterations >= 0; iterations--) {
          ...
          for (let indexQuizResolved = 0; indexQuizResolved < weekData.length; indexQuizResolved++) {
        ...
        if (testTypes === 'verbel') {
          console.log("asdhahdakhdjka akjhd akjsdh", progress.toFixed(2))

          weekWiseProgress.eachCategoryPrognos = verbalPercentageCalculator(progress.toFixed(2));
        } else {
          weekWiseProgress.eachCategoryPrognos = quantitativePercentageCalculator(progress.toFixed(2));
        }
